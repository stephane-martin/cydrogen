...
cdef class SecretBox:
    ...
    cpdef encrypt(self, const unsigned char[:] plaintext, uint64_t msg_id=*, max_msg_size=*)
    cpdef decrypt(self, ciphertext, uint64_t msg_id=*, max_msg_size=*)
    cpdef encrypt_file(self, src, dst, uint32_t chunk_size=*)
    cdef _encrypt_file(self, fileobj, out, uint32_t chunk_size=*)
